...
def main(args):
    ...
    batch_files = [f for f in os.listdir(args.batch_directory) if f.startswith(args.batch_prefix) and f.endswith('.txt')]
    if not batch_files:
        print("No batch ID files found in the specified directory.")
        return
    for batch_file in batch_files:
        with open(os.path.join(args.batch_directory, batch_file), 'r', encoding='utf-8') as f:
        ...
        if batch_status.status == 'completed':
            ...
            retrieve_results(client, output_file_id, index, args.output_prefix, args.output_directory)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch_directory", type=str, required=True, help="Directory containing batch files")
